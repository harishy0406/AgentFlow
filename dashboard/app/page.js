...
export default function Home() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("agentflow_pet_pos");
      if (saved) {
        const parsed = JSON.parse(saved);
        if (typeof parsed?.x === "number" && typeof parsed?.y === "number") {
          const x = Math.max(16, Math.min(window.innerWidth - 80, parsed.x));
          const y = Math.max(16, Math.min(window.innerHeight - 90, parsed.y));
          setPetPos({ x, y });
        }
      }
    } catch (e) { }
  ...
  const handlePetPointerDown = (e) => {
    ...
    const petEl = e.currentTarget.closest(".agent-pet-wrapper");
    if (!petEl) return;
    ...
    const handlePointerMove = (moveEvt) => {
      ...
      const dx = moveEvt.clientX - petDragRef.current.startX;
      const dy = moveEvt.clientY - petDragRef.current.startY;
      if (Math.abs(dx) > 3 || Math.abs(dy) > 3) {
        petDragRef.current.hasMoved = true;
      }
      const newX = Math.max(10, Math.min(window.innerWidth - 76, petDragRef.current.initPetX + dx));
      const newY = Math.max(10, Math.min(window.innerHeight - 86, petDragRef.current.initPetY + dy));
      setPetPos({ x: newX, y: newY });
    };

    const handlePointerUp = () => {
      if (petDragRef.current.isDragging) {
        petDragRef.current.isDragging = false;
        setIsDraggingPet(false);
        if (petDragRef.current.hasMoved) {
          setPetPos((current) => {
            if (current) {
              try { localStorage.setItem("agentflow_pet_pos", JSON.stringify(current)); } catch (err) { }
            }
            return current;
          });
        }
  ...
  return (
    <div className="dashboard">
      {/* Animated Cyber Grid Lines & Floating Ambient Particles Background */}
      <div className="cyber-grid-container" aria-hidden="true">
        <div className="cyber-grid-lines" />
        <div className="cyber-grid-beam" />
        <div className="cyber-ambient-orb orb-1" />
        <div className="cyber-ambient-orb orb-2" />
        <div className="cyber-ambient-orb orb-3" />
        <div className="cyber-float-particle p1" />
        <div className="cyber-float-particle p2" />
        <div className="cyber-float-particle p3" />
        <div className="cyber-float-particle p4" />
        <div className="cyber-float-particle p5" />
        <div className="cyber-float-particle p6" />
      </div>
            ...
            <div>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12, marginBottom: 12 }}>
                  ...
                  <div className="nav-group">
                    <button
                      className={`nav-group-btn ${["create", "projects", "graph", "artifacts", "code", "workspaces"].includes(activeTab)
                          ? "active"
                          : ""
                        }`}
                      onClick={() => {
                    ...
                    <button
                      className={`nav-group-btn ${["drifts", "metrics", "timeline", "evaluations", "security", "load-testing"].includes(activeTab)
                          ? "active"
                          : ""
                        }`}
                      onClick={() => {
                    ...
                    <button
                      className={`nav-group-btn ${["devops", "api-sandbox", "sdks", "webhooks-apm", "changelog-cloudops"].includes(activeTab)
                          ? "active"
                          : ""
                        }`}
                      onClick={async () => {
              ...
              {/* Studio Sub-Tab Active View */}
              <div key={activeTab} className="studio-tab-view">
      ...
      {/* Interactive Floating AI Agent Pet ("Flowy") - Draggable Anywhere */}
      <div
        className={`agent-pet-wrapper ${isDraggingPet ? "dragging" : ""}`}
        style={
          petPos
            ? {
              left: `${petPos.x}px`,
      ...
      >
        <div
          className={`agent-pet-container ${petCheer ? "cheer" : ""} ${isDraggingPet ? "dragging" : ""}`}
        ...
        >
          ...
          {/* Pet Robot Chassis */}
          <div className="pet-chassis">
            <div className="pet-antenna">
              <div className="pet-antenna-light" />
            </div>
            <div className="pet-visor">
              <div className="pet-eye" />
              <div className="pet-eye" />
            </div>
          </div>

          <div className="pet-thruster" />
